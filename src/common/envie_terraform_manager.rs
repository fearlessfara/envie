use crate::common::Result;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::path::Path;
use std::process::Command;
use std::sync::mpsc;
use std::time::Duration;
use terraform::Terraform;

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TerraformOutput {
    pub value: serde_json::Value,
    #[serde(rename = "type")]
    pub output_type: String,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TerraformState {
    pub service: String,
    pub dependencies: Vec<String>,
}

/// Enhanced Terraform manager using the terraform crate for better process management
/// while maintaining the same interface as the original TerraformManager
pub struct EnvieTerraformManager {
    working_directory: std::path::PathBuf,
    verbose: bool,
    terraform_binary: String,
}

impl EnvieTerraformManager {
    pub fn new<P: AsRef<Path>>(working_directory: P) -> Self {
        Self {
            working_directory: working_directory.as_ref().to_path_buf(),
            verbose: false,
            terraform_binary: "terraform".to_string(),
        }
    }

    pub fn with_verbose(mut self, verbose: bool) -> Self {
        self.verbose = verbose;
        self
    }

    pub fn with_binary<P: AsRef<Path>>(mut self, binary_path: P) -> Self {
        self.terraform_binary = binary_path.as_ref().to_string_lossy().to_string();
        self
    }

    pub fn init(&self) -> Result<()> {
        self.run_command("init", &[], false)
    }

    pub fn init_with_upgrade(&self) -> Result<()> {
        self.run_command("init", &["-upgrade"], false)
    }

    pub fn workspace_list(&self) -> Result<Vec<String>> {
        let output = self.run_command_capture("workspace", &["list"], false)?;
        let workspaces: Vec<String> = output
            .lines()
            .map(|line| {
                // Remove the * prefix and trim whitespace
                line.trim().trim_start_matches("* ").to_string()
            })
            .filter(|line| !line.is_empty())
            .collect();
        Ok(workspaces)
    }

    pub fn workspace_show(&self) -> Result<String> {
        let output = self.run_command_capture("workspace", &["show"], false)?;
        Ok(output.trim().to_string())
    }

    pub fn workspace_select(&self, workspace: &str) -> Result<()> {
        self.run_command("workspace", &["select", workspace], false)
    }

    pub fn workspace_new(&self, workspace: &str) -> Result<()> {
        self.run_command("workspace", &["new", workspace], false)
    }

    pub fn workspace_delete(&self, workspace: &str) -> Result<()> {
        self.run_command("workspace", &["delete", workspace], false)
    }

    pub fn apply(&self, vars: &[(&str, &str)]) -> Result<()> {
        let mut args = vec!["-auto-approve", "-input=false"];
        let mut var_args = Vec::new();
        for (key, value) in vars {
            let var_arg = format!("{}={}", key, value);
            var_args.push(var_arg);
        }
        
        for var_arg in &var_args {
            args.extend(&["-var", var_arg]);
        }
        self.run_command("apply", &args, false)
    }

    pub fn apply_with_output(&self, vars: &[(&str, &str)], output_file: &str) -> Result<()> {
        let mut args = vec!["-auto-approve", "-input=false"];
        let mut var_args = Vec::new();
        for (key, value) in vars {
            let var_arg = format!("{}={}", key, value);
            var_args.push(var_arg);
        }
        
        for var_arg in &var_args {
            args.extend(&["-var", var_arg]);
        }
        args.extend(&["-out", output_file]);
        self.run_command("apply", &args, false)
    }

    pub fn destroy(&self, vars: &[(&str, &str)]) -> Result<()> {
        let mut args = vec!["-auto-approve", "-input=false"];
        let mut var_args = Vec::new();
        for (key, value) in vars {
            let var_arg = format!("{}={}", key, value);
            var_args.push(var_arg);
        }
        
        for var_arg in &var_args {
            args.extend(&["-var", var_arg]);
        }
        self.run_command("destroy", &args, false)
    }

    pub fn output_json(&self) -> Result<HashMap<String, TerraformOutput>> {
        let output = self.run_command_capture("output", &["-json"], false)?;
        let parsed: HashMap<String, TerraformOutput> = serde_json::from_str(&output)?;
        Ok(parsed)
    }

    pub fn output_value(&self, key: &str) -> Result<serde_json::Value> {
        let output = self.run_command_capture("output", &["-json", key], false)?;
        let parsed: serde_json::Value = serde_json::from_str(&output)?;
        Ok(parsed)
    }

    /// Run a Terraform command using the terraform crate for better process management
    /// Falls back to Command for commands not supported by the terraform crate
    fn run_command(&self, command: &str, args: &[&str], _quiet: bool) -> Result<()> {
        // For workspace management and other commands not supported by terraform crate,
        // fall back to the original Command approach but with better error handling
        let mut cmd = Command::new(&self.terraform_binary);
        cmd.arg(command);
        cmd.args(args);
        cmd.current_dir(&self.working_directory);

        // Set GODEBUG environment variable as in the original scripts
        cmd.env("GODEBUG", "asyncpreemptoff=1");

        if self.verbose {
            println!(">> Running: {} {} {}", self.terraform_binary, command, args.join(" "));
            // In verbose mode, inherit stdout/stderr to show terraform output
            let status = cmd.status();
            match status {
                Ok(status) => {
                    if status.success() {
                        Ok(())
                    } else {
                        Err(crate::common::EnvieError::TerraformError(
                            format!("{} {} failed with exit code: {}", self.terraform_binary, command, status)
                        ))
                    }
                }
                Err(e) => Err(crate::common::EnvieError::ProcessError(
                    format!("Failed to execute {} {}: {}", self.terraform_binary, command, e)
                )),
            }
        } else {
            // In non-verbose mode, capture output
            let output = cmd.output();
            match output {
                Ok(output) => {
                    if output.status.success() {
                        Ok(())
                    } else {
                        let stderr = String::from_utf8_lossy(&output.stderr);
                        Err(crate::common::EnvieError::TerraformError(
                            format!("{} {} failed: {}", self.terraform_binary, command, stderr)
                        ))
                    }
                }
                Err(e) => Err(crate::common::EnvieError::ProcessError(
                    format!("Failed to execute {} {}: {}", self.terraform_binary, command, e)
                )),
            }
        }
    }

    fn run_command_capture(&self, command: &str, args: &[&str], _quiet: bool) -> Result<String> {
        let mut cmd = Command::new(&self.terraform_binary);
        cmd.arg(command);
        cmd.args(args);
        cmd.current_dir(&self.working_directory);
        
        // Set GODEBUG environment variable as in the original scripts
        cmd.env("GODEBUG", "asyncpreemptoff=1");

        if self.verbose {
            println!(">> Running: {} {} {}", self.terraform_binary, command, args.join(" "));
        }

        let output = cmd.output()?;

        if output.status.success() {
            Ok(String::from_utf8_lossy(&output.stdout).to_string())
        } else {
            let stderr = String::from_utf8_lossy(&output.stderr);
            Err(crate::common::EnvieError::TerraformError(
                format!("{} {} failed: {}", self.terraform_binary, command, stderr)
            ))
        }
    }

    /// Run Terraform init using the terraform crate for better process management
    pub async fn init_async(&self) -> Result<()> {
        let (tx, _rx) = mpsc::channel();
        let mut envs = HashMap::new();
        envs.insert("GODEBUG".to_string(), "asyncpreemptoff=1".to_string());

        let terraform = Terraform::new(
            &self.terraform_binary,
            &self.working_directory,
            envs,
            Duration::from_secs(300), // 5 minute timeout
            tx,
        )?;

        let context = terraform.run_init()?;
        
        if context.exit_code == Some(0) {
            Ok(())
        } else {
            let stderr = context.stderr.join("\n");
            Err(crate::common::EnvieError::TerraformError(
                format!("terraform init failed: {}", stderr)
            ))
        }
    }

    /// Run Terraform apply using the terraform crate for better process management
    pub async fn apply_async(&self, vars: &[(&str, &str)], plan_file: &str) -> Result<()> {
        let (tx, _rx) = mpsc::channel();
        let mut envs = HashMap::new();
        envs.insert("GODEBUG".to_string(), "asyncpreemptoff=1".to_string());

        let terraform = Terraform::new(
            &self.terraform_binary,
            &self.working_directory,
            envs,
            Duration::from_secs(1800), // 30 minute timeout
            tx,
        )?;

        // First create a plan
        let plan_context = terraform.run_plan(&plan_file.to_string())?;
        
        if plan_context.exit_code != Some(0) {
            let stderr = plan_context.stderr.join("\n");
            return Err(crate::common::EnvieError::TerraformError(
                format!("terraform plan failed: {}", stderr)
            ));
        }

        // Then apply the plan
        let apply_context = terraform.run_apply(&plan_file.to_string())?;
        
        if apply_context.exit_code == Some(0) {
            Ok(())
        } else {
            let stderr = apply_context.stderr.join("\n");
            Err(crate::common::EnvieError::TerraformError(
                format!("terraform apply failed: {}", stderr)
            ))
        }
    }

    /// Run Terraform destroy using the terraform crate for better process management
    pub async fn destroy_async(&self) -> Result<()> {
        let (tx, _rx) = mpsc::channel();
        let mut envs = HashMap::new();
        envs.insert("GODEBUG".to_string(), "asyncpreemptoff=1".to_string());

        let terraform = Terraform::new(
            &self.terraform_binary,
            &self.working_directory,
            envs,
            Duration::from_secs(1800), // 30 minute timeout
            tx,
        )?;

        let context = terraform.run_destroy()?;
        
        if context.exit_code == Some(0) {
            Ok(())
        } else {
            let stderr = context.stderr.join("\n");
            Err(crate::common::EnvieError::TerraformError(
                format!("terraform destroy failed: {}", stderr)
            ))
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use tempfile::TempDir;

    #[test]
    fn test_envie_terraform_manager_creation() {
        let temp_dir = TempDir::new().unwrap();
        let manager = EnvieTerraformManager::new(temp_dir.path());
        assert_eq!(manager.working_directory, temp_dir.path());
        assert!(!manager.verbose);
        assert_eq!(manager.terraform_binary, "terraform");
    }

    #[test]
    fn test_envie_terraform_manager_with_verbose() {
        let temp_dir = TempDir::new().unwrap();
        let manager = EnvieTerraformManager::new(temp_dir.path()).with_verbose(true);
        assert!(manager.verbose);
    }

    #[test]
    fn test_envie_terraform_manager_with_binary() {
        let temp_dir = TempDir::new().unwrap();
        let manager = EnvieTerraformManager::new(temp_dir.path()).with_binary("/custom/terraform");
        assert_eq!(manager.terraform_binary, "/custom/terraform");
    }
}
